...
function joinOrderJson(request, response) {
  ...
  connection.query("SELECT STUDENT_ID, COUNT(STUDENT_ID) AS ORDERS_THIS_MONTH FROM `ORDER` WHERE MONTH(DATE) = ? AND YEAR(DATE) = ? GROUP BY STUDENT_ID", [month, year], function(err, rows, fields) {
    ...
    connection.query("SELECT `ORDER`.ID, `ORDER`.STUDENT_ID, `ORDER`.PHONE, `ORDER`.EMAIL, `ORDER`.DATE, `ORDER`.VETERAN, `ORDER`.DISABLED, `ORDER`.SNAP, `ORDER`.HOUSEHOLD, `ORDER`.PACKAGED, `ORDER`.PICKEDUP, `ORDER`.CANCELED, INVENTORY.FoodGroup, INVENTORY.FoodName FROM `ORDER`, ORDER_ITEM, INVENTORY WHERE `ORDER`.ID = ORDER_ITEM.ORDER_ID AND ORDER_ITEM.INVENTORY_ID = INVENTORY.ID ORDER BY `ORDER`.ID DESC", function(err, rows, fields) {
      var json = {};
      if (err) {
        json["success"] = false;
